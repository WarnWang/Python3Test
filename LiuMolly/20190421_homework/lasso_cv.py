...
if __name__ == '__main__':
    ...
    x_variable_selection = ['MKT_CAP', 'RETVOL', 'CHMOM', 'MAXRET', 'DOLVOL']
    ...
    top10returns = np.zeros((test_period, 1))
    LS10returns = np.zeros((test_period, 1))
    rankcorr = np.zeros((test_period, 1))
    returncorr = np.zeros((test_period, 1))
    market_returns = np.zeros((test_period, 1))
    ...
    for t in trange(T - test_period, T):
        ...
        method = LassoCV(cv=5, random_state=0).fit(train_x, train_y)
        ...
        top10returns[tt, 0] = t_top10return
        LS10returns[tt, 0] = t_LS_10return
        rankcorr[tt, 0] = t_rankcorr
        returncorr[tt, 0] = t_return_corr
        market_returns[tt, 0] = t_marketreturn

    result_df: pd.DataFrame = pd.DataFrame([top10returns, LS10returns, rankcorr, returncorr, market_returns],
