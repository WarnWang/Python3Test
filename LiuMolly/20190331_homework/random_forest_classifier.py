...
def get_next_return_rank(df):
    df.loc[:, RETURN_1] = df[RETURN].shift(-1)
    df.loc[:, REAL_RANK] = df[RETURN_1].rank(ascending=False, method='dense')
    return df
...
if __name__ == '__main__':
    ...
    data_df_add_next_return = data_df.groupby(STOCK_SYMBOL).progress_apply(get_next_return_rank).reset_index(drop=True)
    ...
    for date_index in tqdm(range(25, 1, -1)):
        ...
        test_data_df.loc[:, PREDICT_RANK] = reg.predict(test_data_df[x_vars])
